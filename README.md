...
`managementPolicy: Observe` turns an Artifact into a sensor: the controller
computes the same content-addressed key, verifies existence, and tracks drift
— but never runs a generator and never writes to the store. A missing
artifact is `Ready=False` / `state: Missing`, not a remediation trigger, and
a `Regenerate` drift policy degrades to `Warn`.

Use it for artifacts some other system owns (another team's pipeline, a
vendor upload) when what you want in-cluster is the condition: anything that
can gate on `Ready` — ArgoCD health, sync waves, a Job that waits for an
upstream image — can now gate on "this exists in its store". It is also the
adoption on-ramp: point an observe-only Artifact at objects an existing
pipeline produces, watch it go green, then flip the field to `Full` to hand
the controller the keys (the field is deliberately mutable).

Because provenance stamps are hashes of the *identity* and not of the CR, an
observe-only Artifact in one namespace goes `Ready` on an object generated by
a `Full` Artifact with the same identity in another — cross-namespace
consumers need no coordination beyond agreeing on the identity keys.

Observe-only classes may omit `generator` entirely. The CRD rejects
`deleteAfter` and `deletionPolicy: Delete` on observe-only Artifacts, since
both delete from the store (`ttl` remains valid — it only deletes the CR).

```yaml
spec:
  classRef:
    name: s3-release-evidence   # a class with store: and no generator:
...
helm install artifact-controller oci://ghcr.io/kargops/charts/artifact-controller \
  -n artifact-system --create-namespace   # add --version X.Y.Z to pin
```
...
pipeline, an HTTP-driver class for Nexus, an Entra ID account observed via
Microsoft Graph (read its header warning before pointing it at people), and a
generator-less observe-only pair (`observe-only.yaml`).
