...
has no planning gate and no readiness labels (`docs/agent-workflow.md`, `.github/labels.yml`), so
the usual reason the queue is empty is a candidate that is technically sound but waits on one
bounded human input: a maintainer decision the issue leaves open (for example, which
`driftPolicy`/`managementPolicy` behavior a new case should get), or reporter information a bug
report omitted (controller version, the class, or the Artifact's `status` from the bug-report
form). That is the cheapest exclusion to clear.

So whenever the selection is `SELECTED: none` **and at least one candidate was excluded solely for
one such open decision or missing report detail**, end the report with a `DECISION GAP` section
naming exactly one issue — the one whose answer would return the most value soonest. Judge that on
the same simplicity ranking used for selection, then break ties on the README roadmap order
("Roadmap, roughly in order of value") and on how much of the change surface the issue body
already pins down.
...
Blocked only by: <the single open decision or missing report detail, stated as a question>
Asked in: <the drafted or existing concern comment that poses it>
Labels: <type/*, area/*>
...
dependency, or scope beyond one bounded change. A single open decision is the *only* exclusion
this section covers; do not use it to relitigate an issue's technical defects, and do not use it
...
This section is still only a report. Naming the gap does not authorize posting, answering, or
implementing it — the read-only rule above is unchanged, and running `/implement-issue` is the
caller's decision.
